...
CONTROL_MODE: ControlMode = ControlMode.HybridJointExtTorqueImpedance
GRIPPER_SPEED = 0.7
GRIPPER_FORCE = 0.3
GRIPPER_DEADBAND = 1e-3


class TrajectoryTorquePandaControlPair(ControlPair):
    def __init__(
        self,
        trajectory_dir: Path,
        ...
        self.ext_torque: Optional[np.ndarray] = None
    ...
    def load_trajectory(self) -> None:
        ...
        ext_torque_path = self.trajectory_dir / "external_joint_torque.pt"
        ...
        if not ext_torque_path.exists():
            raise FileNotFoundError(f"Missing replay external joint torque trajectory: {ext_torque_path}")

        joint_pos = torch.load(joint_pos_path, map_location="cpu")
        gripper_pos = torch.load(gripper_state_path, map_location="cpu")
        ext_torque = torch.load(ext_torque_path, map_location="cpu")
        ...
        self.ext_torque = np.asarray(ext_torque, dtype=np.float64).reshape(-1, 7)
        ...
        if self.ext_torque.shape[0] == 0:
            raise ValueError(f"Empty replay external joint torque trajectory: {ext_torque_path}")

        replay_len = min(
            self.joint_pos.shape[0],
            self.gripper_pos.shape[0],
            self.ext_torque.shape[0],
        )
        self.joint_pos = self.joint_pos[:replay_len]
        self.gripper_pos = self.gripper_pos[:replay_len]
        self.ext_torque = self.ext_torque[:replay_len]
        self._step_idx = 0
    ...
    def save_trajectory(self, output_dir: Path, target_len: Optional[int] = None) -> None:
        if self.joint_pos is None or self.gripper_pos is None or self.ext_torque is None:
            self.load_trajectory()

        assert self.joint_pos is not None
        assert self.gripper_pos is not None
        assert self.ext_torque is not None
        ...
        if target_len is not None:
            ...
            joint_pos, gripper_pos, ext_torque = self._resize_trajectory(
                joint_pos, gripper_pos, ext_torque, target_len
            )
        ...
        torch.save(
            torch.tensor(ext_torque, dtype=torch.float64),
            output_dir / "external_joint_torque.pt",
        )
    ...
    def _resize_trajectory(
        ...
        if target_len <= joint_pos.shape[0]:
            return (
                joint_pos[:target_len],
                gripper_pos[:target_len],
                ext_torque[:target_len],
            )
        ...
        torque_padding = np.repeat(ext_torque[-1:], pad_len, axis=0)
        return (
            np.concatenate([joint_pos, joint_padding], axis=0),
            np.concatenate([gripper_pos, gripper_padding], axis=0),
            np.concatenate([ext_torque, torque_padding], axis=0),
        )
    ...
    def control_reset(self) -> None:
        ...
        self.follower.panda_arm.send_joint_torque_command(np.zeros(7, dtype=np.float64))
    ...
    def _control_task(self) -> None:
        try:
            ...
            try:
                ...
                print(f"Control task cleanup encountered an error: {e}")
