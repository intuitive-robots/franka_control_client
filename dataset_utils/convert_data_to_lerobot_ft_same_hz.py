...
def save_episode_to_lerobot(
    ...
    state_np = np.concatenate([follower_joint_np[:-1], follower_grip_np[:-1]], axis=1)  # [T-1,8]
    force_torque_np = np.concatenate(
        [
            follower_external_joint_torque_np[:-1],
            follower_external_wrench_np[:-1],
            follower_gripper_current_np[:-1],
        ],
        axis=1,
    )  # [T-1,14]
